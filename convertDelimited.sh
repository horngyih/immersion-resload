...
do
	...
	then
		...
		head -n -3 $FILE | awk -v widths="$COLUMNS2" -v source="$FILENAME" -f $BASEDIR/res-roomtype.awk > $FILE.csv
	else
		echo $FILENAME - $HAS_ROOM_TYPE - $COLUMNS1;
		head -n -3 $FILE | awk -v widths="$COLUMNS1" -v source="$FILENAME" -f $BASEDIR/res.awk > $FILE.csv
	fi
done;
